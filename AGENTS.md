...
- The `gastos` plugin stores servicios/pagos/credenciales/queries in local JSON (`userData`) or Supabase when migrated. Optional Notion sync; config files `gastos-notion.local.example.json`, `gastos-supabase.local.example.json`. See `docs/SUPABASE_MIGRATION.md`.
